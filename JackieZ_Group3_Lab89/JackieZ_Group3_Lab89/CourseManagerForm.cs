    ...
    public partial class CourseManagerForm : Form
    {
        ...
        private void btn_CourseAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                { Name = txt_CourseName.Text, Semester = Convert.ToByte(txt_CourseSem.Text) };
        ...
        {
            ViewEvaluationsForm viewEvaluationsForm = new ViewEvaluationsForm(currentlySelectedCourse);
            if (viewEvaluationsForm.ShowDialog() == DialogResult.Cancel)
            {
                RefreshCourseList();
            }
        }
